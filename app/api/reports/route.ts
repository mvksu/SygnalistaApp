  ...
  try {
    ...
    const [inserted] = await db
      ...
      .values({
        ...
        status: "NEW",
        createdAt: now,
        ackDueAt: (org?.slaEnabled as boolean ?? true) ? new Date(now.getTime() + (((org?.ackDays as number) ?? 7) * 24 * 60 * 60 * 1000)) : null,
        feedbackDueAt: (org?.slaEnabled as boolean ?? true) ? addMonths(now, ((org?.feedbackMonths as number) ?? 3)) : null,
        reporterMode,
